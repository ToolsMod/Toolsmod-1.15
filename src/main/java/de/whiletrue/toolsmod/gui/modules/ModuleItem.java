...
public class ModuleItem extends MultirowListItem<Module>{
	...
	public void render(int mX, int mY, float ticks, TmWidget focused) {
		...
		RENDERER.renderRect(this.x, this.y, this.width, this.height, hovered?0xff008FFF:0xFF000000);
		//Renders the text
		RENDERER.renderCenteredString(this.item.getName(), this.x+this.width/2, this.y+this.height/2-4, this.item.isActive()?0xff1DC617:0xffC61717);
	}
