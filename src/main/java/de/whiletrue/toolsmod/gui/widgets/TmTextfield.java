...
public class TmTextfield extends TmSizeWidget {
	...
	public Optional<TmWidget> onMouseClicked(double mX, double mY, int ticks) {
		...
		String trimmed = this.mc.fontRenderer.trimStringToWidth(this.text.substring(this.lineScrollOffset),
				this.getAdjustedWidth());
		// Sets the cursor
		this.setCursorPosition(this.mc.fontRenderer.trimStringToWidth(trimmed, i).length() + this.lineScrollOffset);
	...
	public void onRender(int mX, int mY, float ticks, TmWidget focused) {
		...
		if (this.enableBackgroundDrawing) {
			AbstractGui.fill(this.x - 1, this.y - 1, this.x + this.width + 1, this.y + this.height + 1, -6250336);
			AbstractGui.fill(this.x, this.y, this.x + this.width, this.y + this.height, -16777216);
		}
		...
		String s = this.mc.fontRenderer.trimStringToWidth(render.substring(this.lineScrollOffset),
				this.getAdjustedWidth());
		...
		if (!s.isEmpty()) {
			...
			j1 = this.mc.fontRenderer.drawStringWithShadow(this.textFormatter.apply(s1, this.lineScrollOffset),
					(float) l, (float) i1, i);
		}
		...
		if (!s.isEmpty() && flag && j < s.length()) {
			this.mc.fontRenderer.drawStringWithShadow(this.textFormatter.apply(s.substring(j), this.cursorPosition),
					(float) j1, (float) i1, i);
		}
		...
		if (flag1) {
			if (flag2) {
				...
				this.mc.fontRenderer.drawStringWithShadow("_", (float) k1, (float) i1, i);
			}
		}
	...
	public void setSelectionPos(int position) {
		...
		if (this.mc.fontRenderer != null) {
			...
			String s = this.mc.fontRenderer.trimStringToWidth(this.text.substring(this.lineScrollOffset), j);
			int k = s.length() + this.lineScrollOffset;
			if (this.selectionEnd == this.lineScrollOffset)
				this.lineScrollOffset -= this.mc.fontRenderer.trimStringToWidth(this.text, j, true).length();
