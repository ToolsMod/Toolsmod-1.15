...
public class ConfigSwitchButton extends TmButton {
	...
	public void onRender(int mX, int mY, float ticks, TmWidget focused) {
		...
		this.renderer.renderRect(this.x, this.y+less, this.width, this.height-less*2, this.backgroundColor);
		
		//Renders the outer lines
		this.renderer.renderRect(this.x,this.y+less,1,this.height-less*2,this.lineColor);
		this.renderer.renderRect(this.x+this.width-1,this.y+less,1,this.height-less*2,this.lineColor);
		
		//Renders the string
		if(this.hoverText!=null && this.normalText!=null)
			this.renderer.renderCenteredString(this.constandUnderlined||focused == this||this.hovered?this.hoverText:this.normalText, this.x+this.width/2, this.y+this.height/2-this.font.FONT_HEIGHT/2, 0xffffffff);
	}
