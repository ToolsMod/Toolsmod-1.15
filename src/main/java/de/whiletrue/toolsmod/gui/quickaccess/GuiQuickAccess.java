...
public class GuiQuickAccess extends TmScreen {
	...
	protected void renderQuickAccessGui() {
		...
		this.renderer.renderRect(this.width / 2 - currentX / 2, this.height / 2 - currentY / 2, currentX, currentY,
				0xff1e252f);
		// Renders the title-background
		this.renderer.renderRect(this.width / 2 - currentX / 2, this.height / 2 - currentY / 2, currentX, 20,
				0xFF000000);
		// Button-background
		this.renderer.renderRect(this.width / 2 - currentX / 2, this.height / 2 - currentY / 2, 50, currentY,
				0xFF000000);
		// Renders the title
		this.drawCenteredString(this.font, this.guiTitle, this.width / 2 + 25, this.height / 2 - currentY / 2 + 6,
				0xFFFFFFFF);
		// Iterates over every menu button
		for (int i = 0; i < this.group.getSize(); i++) {
			// Renders the button
			this.renderer.renderRect(this.width / 2 - currentX / 2, this.height / 2 - currentY / 2 + 20 + i * 20, 50,
					20, i == this.group.getIndex() ? 0xffEE5A24 : 0xFF000000);
			// Renders the text
			this.drawCenteredString(this.font, this.group.getGui(i).getGuiTitleShort(),
