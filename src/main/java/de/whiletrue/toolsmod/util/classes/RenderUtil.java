...
public class RenderUtil {
	...
	public void renderRect(int x,int y,int w,int h,int color) {
		AbstractGui.fill(x,y,x+w,y+h,color);
	}
	...
	public void renderCenteredString(String text,int x,int y,int color) {
		this.mc.fontRenderer.drawStringWithShadow(text, x-this.mc.fontRenderer.getStringWidth(text)/2, y, color);
	}
    ...
    public void renderImage(ResourceLocation image,int x,int y,int width,int height) {
		...
		//Renders the image
		AbstractGui.blit(x, y, 0, 0, 0, width, height,width,height);
    }
}
