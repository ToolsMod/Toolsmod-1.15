package de.whiletrue.toolsmod.module.defined.special.asedit.gui;
...
public abstract class GuiAsEdit extends TmScreen{
	...
	public void render(int mX, int mY, float ticks) {
		...
		fill(0, 0, this.overlayX, this.height, 0x9f000000);
		...
		for(int i=0;i<this.module.guis.getSize();i++) {
			...
			//Renders the field
			fill(0,y,this.overlayX,y+20,this.module.guis.getSelected().equals(gui)?0xffEE5A24:0xFF000000);
			
			//Renders the name
			drawCenteredString(this.minecraft.fontRenderer, gui.getTitle().getFormattedText(), this.overlayX/2, y+6, 0xFFffffff);
		}
	}
