...
public class AsEditManager {
	...
	public void doTeleportToPlayer() {
    	...
    	//Gets the middle position
    	Vec3d middle = selected[0].getPositionVec();
    	
    	//Gets the player position
    	Vec3d pos = this.game.player.getPositionVec();
    	
    	//Teleport's all stands in relation to the stand
    	for(EditableArmorStand s : selected) {
    		//Calculates the position
    		Vec3d sdiff = s.getPositionVec().subtract(middle);
    		
    		Vec3d diff = pos.add(sdiff);
     ...
     */
    public Vec3d getGlobalRotations() {
		...
		return new Vec3d(rot.getX(), rot.getY(), rot.getZ());
    }
