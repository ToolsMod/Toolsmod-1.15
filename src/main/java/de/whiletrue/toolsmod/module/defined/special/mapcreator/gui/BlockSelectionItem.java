package de.whiletrue.toolsmod.module.defined.special.mapcreator.gui;
...
public class BlockSelectionItem extends MultirowListItem<BlockSelection>{
	...
	public void render(int mX, int mY, float ticks, TmWidget focused) {
		...
		RENDERER.renderRect(this.x, this.y, this.width, this.height, this.selected?0xffee5a24:hovered?0xff008FFF:0xff707070);
		
		//Renders the selected block
		RENDERER.renderItem(this.is, this.model, this.x+2, this.y+this.height/2-8);
		
		//Renders the color
		RENDERER.renderRect(this.x+this.width-14, this.y+this.height/2-6, 12, 12,this.item.getColor().getRGB());
	}
